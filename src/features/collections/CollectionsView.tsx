            ...
            <TableBody>
              {isLoading ? (
                ...
                filteredCollections.map((collection) => (
                  <TableRow key={collection.id} className="border-zinc-100 hover:bg-zinc-50/50 transition-colors">
                    <TableCell className="font-medium text-zinc-900 pl-6 py-4">{collection.name}</TableCell>
                    <TableCell className="text-zinc-500 py-4">{collection.slug}</TableCell>
                    <TableCell className="text-zinc-500 py-4 text-center">{collection.position}</TableCell>
                    <TableCell className="py-4">
                      <span className={"inline-flex items-center px-2 py-1 rounded-md text-[11px] font-medium " + (collection.is_active ? 'bg-emerald-50 text-emerald-600' : 'bg-zinc-100 text-zinc-500')}>
                        {collection.is_active ? 'Activa' : 'Inactiva'}
                      </span>
                    </TableCell>
                    <TableCell className="py-4 pr-6 text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => openEditSheet(collection)} className="w-8 h-8 rounded-lg hover:bg-zinc-100 text-zinc-500 hover:text-zinc-900">
                          <Pencil className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => setDeleteConfirm(collection.id)} className="w-8 h-8 rounded-lg hover:bg-rose-50 text-zinc-500 hover:text-rose-600">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex-1 overflow-y-auto p-6 space-y-6">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input {...register('name')} onChange={handleNameChange} className="rounded-xl bg-zinc-50 border-zinc-200" placeholder="Ej. Verano 2025" />
              ...
              <div className="space-y-2">
                ...
                <Input {...register('slug')} className="rounded-xl bg-zinc-50 border-zinc-200 text-zinc-500" placeholder="verano-2025" />
              ...
              <div className="flex items-center space-x-2 pt-2">
                <input type="checkbox" id="is_active_collection" {...register('is_active')} className="rounded border-zinc-300 text-primary focus:ring-primary h-4 w-4" />
                <label htmlFor="is_active_collection" className="text-sm font-medium text-zinc-900">
                  Colección Activa
                </label>
              </div>
            </div>
